...
// Urls
var apiUrl = "https://pokevision.com/map/data/";
var imageUrl = "https://ugc.pokevision.com/images/pokemon/";
...
// Utils
var getLocalization = function (callback) {
...
var radar = function (lat, lon, callback) {
  var info = apiUrl + lat + "/" + lon;
  ajax({ url: info, method: 'get', type: 'json' },
    function(data) {
      if (data.status === "success") {
        return callback(null, data.pokemon);
      } else {
        return callback("Failed obtaining the pokémon.", null);
      }
    },
    function (err) { return callback("Failed obtaining the data from server.", null); }
  );
};
...
var populateList = function (data) {
  ...
  for (var i in data) {
    var time = new Date(data[i].expiration_time);
    var diffMs = (timeNow - time);
    var diffMins = Math.round(((diffMs % 86400000) % 3600000) / 60000);
    var p = {
      title: pokemon[data[i].pokemonId - 1],
      icon: imageUrl + data[i].pokemonId + ".png",
...
var findNear = function (callback) {
  getLocalization(function (err, cords) {
    ...
    radar(cords.latitude, cords.longitude, function (err, res) {
      if (err) { return callback(err, null); }
      var pkmn = populateList(res);
      return callback(null, pkmn);
    });
  });
};
